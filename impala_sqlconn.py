"""cdw-impala-sql — end-to-end CRUD DAG for Impala over CDW.

Multi-step trigger-only DAG driven by cwo-qe (see
tests/functional/providers/test_cloudera_providers.py::test_cdw_impala_sql_dag_run_succeeds).

Same shape as hive_sqlconn.py; differs only in dag_id and CONN_ID.
Impala uses the same partitioned-table + INSERT OVERWRITE PARTITION idiom
Hive does for updates.
