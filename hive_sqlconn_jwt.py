...
with DAG(
    ...
    create_table_impala = SQLExecuteQueryOperator(
        ...
        conn_id="cdw-hive-sql-jwt",
        sql="""
                CREATE TABLE IF NOT EXISTS test_hive.example_hive_jwt (
                    a STRING,
                    b INT
                )
    ...
    write_table_impala = SQLExecuteQueryOperator(
        task_id="write_table",
        conn_id="cdw-hive-sql-jwt",
        sql="INSERT INTO test_hive.example_hive_jwt PARTITION (c)  VALUES ('test1', 1, 2)",
    )

    read_table_impala = SQLExecuteQueryOperator(
        task_id="read_table",
        conn_id="cdw-hive-sql-jwt",
        sql="SELECT * FROM test_hive.example_hive_jwt",
    )
